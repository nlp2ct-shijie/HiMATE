...
class MultiAgentPrompting_SRv:
    def __init__(self, dataset, lang_pair, translation_system, agent_type, dataset_prefix, agent_type_prefix, error_exist_lst, messages_path, process_log_path):
        '''
        ...
        self.agent_type_file_path = os.path.join(agent_type_prefix, lang_pair, f'description_SRv.json')
    ...
    def generate_prompt(self):
        ...
        with open(os.path.join(self.dataset_file_path, 'hyp.txt'), 'r', encoding='utf-8') as hyps:
            output_file_path = self.messages_path
            process_log_lines = read_json(self.process_log_path)
            messages_lst = []
            for (raw_translation, corrected_translation_line, error_exist) in zip(hyps, process_log_lines, self.error_exist_lst):
                if error_exist[0] == 'Yes':
                    prompt = 'Error definition: ' + str(agent_type_description)
                    prompt += 'Original Translation: ' + raw_translation + '\n'
                    prompt += 'Corrected Translation: ' + corrected_translation_line[1]["Corrected Translation"] + '\n'
                    prompt += reply_format
                    message_lst = [
