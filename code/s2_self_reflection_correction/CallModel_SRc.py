...
class CallModel_SRc:
    def __init__(self, model, messages_path, responses_path, responses_log_path, base_url, api_key, error_exist_lst, next_id=0, max_tokens=500,
                 temperature=0, stop=None, seed=42, tools=None, logprobs=None, top_logprobs=None):
        '''
        model: the model used for the LLM agent, such as gpt-4
        messages_path: the path of the messages to be called
        responses_path: the path of the responses to be saved
        responses_log_path: the path of the response logs to be saved
        base_url: the base url of the used model
        api_key: the key of the used model
        error_exist_lst: the list about whether the error exists
        next_id: the next dataset id to be called
        '''
        self.model = model
        ...
        while flag:
            ...
            with tqdm(total=range_max - next_id, desc=f"Call {self.model}") as pbar:
                ...
                while current_i < range_max:
                    ...
                    if 'Yes' in self.error_exist_lst[current_i]:
                        while retries > 0:
                            try:
